  ...
  blocks: [
    ...
    },
    {
      ...
      items: [
        ...
        { term: "Claude Code", md: "The agent host Sphynx runs inside. There is no separate backend server or Python orchestrator, the PM *is* the session. See [Architecture](/docs/architecture)." },
        { term: "MCP (Model Context Protocol)", md: "The protocol that connects Claude Code to the broker. Sphynx uses one MCP server, `robinhood-trading`. See [MCP & Tools](/docs/mcp)." },
    ...
    },
    {
      ...
      items: [
        ...
        { term: "Prompt injection", md: "External content that tries to act like an instruction (\"buy X now\", \"ignore your rules\"). Sphynx treats all external content as untrusted data." },
      ],
    },
    {
      type: "heading",
    ...
    },
    {
      type: "deflist",
      items: [
        { term: "Robinhood Chain", md: "An Arbitrum Orbit rollup, chainId **4663**, gas in ETH. Where the SPHYNX contracts and the Stock Token pools live. RPC `rpc.mainnet.chain.robinhood.com`, explorer `robinhoodchain.blockscout.com`." },
        { term: "USDG", md: "Global Dollar, the 6-decimal stablecoin the vault is denominated in (`0x5fc5…d168`). An 18-decimal look-alike exists on the chain; the deploy script pins 6." },
        { term: "Stock Token", md: "A Robinhood-issued ERC-20 that tracks a stock's price (NVDA, AAPL, SPY…). 18 decimals. **Not a share**, no voting rights, **not for US persons**. The vault holds these." },
        { term: "RWAVault / vSPHYNX", md: "The ERC-4626 vault over USDG that holds Stock Tokens. vSPHYNX is its share (12 decimals). Reverts any order that breaches the caps. See [The Vault](/docs/vault)." },
        { term: "previewTrade()", md: "The free view call that returns the first rule an order would break, or `None`. The same check runs inside `executeTrade()`. The site's [Trade terminal](/trade) calls it as you type." },
        { term: "Violation", md: "The enum `previewTrade()` returns: `None`, `Unfunded`, `DailyLossHalt`, `PerTradeCap`, `MaxDailyOrders`, `Concentration`, `MaxPositions`, `CashBuffer`, `NoAveragingIntoLoser`, `MissingStop`, `NotAllowed`, `ZeroAmount`, `InsufficientPosition`, `Paused`. Table in the [Trading Guide](/docs/trading)." },
        { term: "GuardrailConfig", md: "The caps as chain state, read by the vault on every order. Owner-changeable within hard ceilings nobody can widen (per-trade ≤ 50%, stop ≤ 25%, cash buffer ≥ 5%)." },
        { term: "SessionKeyExecutor", md: "The vault's only trader. Holds one session per agent key: expiry, per-trade notional, budget, trade count, sides, ticker allowlist. A refused order reverts and spends none of it. See [Session Keys](/docs/session-keys)." },
        { term: "Session", md: "The scoped, expiring permission an agent key holds on the executor. `isLive()` is true while it is active, unexpired, and has count and budget left." },
        { term: "TWAP", md: "Time-weighted average price. The oracle's mark is the 5-minute TWAP from the token's Uniswap V3 pool; it cannot be moved inside one block." },
        { term: "Deviation bound", md: "If spot is more than 3% from the TWAP the oracle reverts, which freezes valuation and orders until the pool settles. `redeemInKind` is unaffected." },
        { term: "UniswapV3Adapter", md: "The vault's only execution surface: swaps straight against one owner-registered USDG pool per token. No router, no path, no arbitrary call." },
        { term: "minAmountOut", md: "The slippage floor on an order, set by the caller. Checked by the adapter and again by the vault's own balance delta." },
        { term: "redeemInKind()", md: "Burn shares for a pro-rata slice of cash **and every token**, minus the exit fee. Always works: while paused, with a broken oracle, without the owner." },
        { term: "Exit fee", md: "0% at launch, hard-capped at 1%. Stays in the vault for remaining holders. The only fee in the contracts; there is no management or performance fee." },
        { term: "DeskRegistry", md: "Append-only attestations (epoch, NAV, PnL, snapshot hash, URI) keyed by a subject the attester owns. Nothing can be edited or removed. Where refusals get published." },
        { term: "Deposit cap", md: "10,000 USDG at launch. An owner-changeable setting that bounds the blast radius until the audit and the Safe are done." },
        { term: "Ownable2Step", md: "Ownership transfer that the new owner must accept, so a typo cannot hand the contracts to a dead address. Every SPHYNX contract with an owner uses it." },
        { term: "Timelock", md: "A delay between an owner signing a change and it taking effect. **Not deployed yet.** The first planned upgrade; see [Risks](/docs/risks)." },
      ],
    },
    {
      type: "note",
